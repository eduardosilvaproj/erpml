  ...
  const reprocessarTodas = async () => {
    ...
    try {
      for (const inv of invoicesWithPending) {
        const r = await invoicesService.reprocessInvoiceStock(inv.id, companyId);
...
function DetailDialog({ invoiceId, onClose }: { invoiceId: string | null; onClose: () => void }) {
  ...
  const reprocessar = async () => {
    ...
    try {
      const result = await invoicesService.reprocessInvoiceStock(data.id, companyId);
      toast({
        title: result.pendingCount > 0 || result.skippedCount > 0 ? "Reprocessamento com pendências" : "Reprocessamento concluído",
        description: `${result.createdCount} criado(s), ${result.updatedCount} atualizado(s)${result.skippedCount ? `, ${result.skippedCount} ignorado(s)` : ""}${result.pendingCount ? `, ${result.pendingCount} pendente(s)` : ""}.`,
      });
      await refetch();
      queryClient.invalidateQueries({ queryKey: ["products"] });
      queryClient.invalidateQueries({ queryKey: ["entrada-nota-historico"] });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (conferences && conferences.length > 0) {
        ...
        const { error: delCiErr } = await supabase.from('conference_items').delete().in('conference_id', confIds).eq('company_id', companyId as string);
        if (delCiErr) throw delCiErr;
        const { error: delConfErr } = await supabase.from('conferences').delete().eq('invoice_id', invoice.id).eq('company_id', companyId as string);
