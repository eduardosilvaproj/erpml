...
export interface BipagemItemState {
  ...
  scannedQty: number;
  status: ItemStatus;
  // Kit support: linha pode representar um kit em vez de um produto avulso
  kitId?: string | null;
  isKit?: boolean;
...
export const useOrdensFull = () => {
  ...
  return useQuery({
    ...
    enabled: !!companyId,
    queryFn: () => ordersService.fetchOrdensFull(companyId as string),
  });
};
...
export const useOrdemFull = (ordemId: string | null) => {
  ...
  return useQuery({
    ...
    queryFn: () => ordersService.fetchOrdemFull(ordemId as string, companyId as string),
  });
};
...
export const useUpdateOrdemStatus = () => {
  ...
  return useMutation({
    mutationFn: ({ id, status, extra }: { id: string; status: OrdemStatus | string; extra?: Record<string, Json> }) => {
      if (!companyId) throw new Error("Empresa não identificada");
      return ordersService.updateOrdemStatus(id, status as OrdemStatus, companyId as string, extra);
    },
    onSuccess: (_, v) => {
...
export const useUpdateItemQuantity = () => {
  ...
  return useMutation({
    mutationFn: (params: { itemId: string; qtd_separada: number; qtd_solicitada: number; orderId?: string }) => {
      if (!companyId) throw new Error("Empresa não identificada");
      return ordersService.updateItemQuantity(params, companyId as string);
    },
    onSuccess: (data) => {
...
export const useDeleteOrdem = () => {
  ...
  return useMutation({
    mutationFn: ({ id }: { id: string; frete_ml?: string | null }) => {
      if (!companyId) throw new Error("Empresa não identificada");
      return ordersService.deleteOrdem(id, companyId as string);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["ordens-full"] });
...
export const useUpdateFullOrder = () => {
  ...
  return useMutation({
    mutationFn: ({ id, status, ...rest }: { id: string; status: string; [key: string]: Json }) => {
      if (!companyId) throw new Error("Empresa não identificada");
      return ordersService.updateOrdemStatus(id, status as OrdemStatus, companyId as string, rest);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["full-orders", companyId] });
...
export const useMarcarOrdemEnviada = () => {
  ...
  return useMutation({
    mutationFn: (ordemId: string) => {
      if (!companyId) throw new Error("Empresa não identificada");
      return ordersService.updateOrdemStatus(ordemId, "enviado", companyId as string);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["ordens-full"] });
    },
...
export const useConcluirOrdem = () => {
  ...
  return useMutation({
    mutationFn: (ordemId: string) => {
      if (!companyId) throw new Error("Empresa não identificada");
      return ordersService.updateOrdemStatus(ordemId, "concluida", companyId as string);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["ordens-full"] });
    },
...
export const useMarcarOrdemSeparada = () => {
  ...
  return useMutation({
    mutationFn: (ordemId: string) => {
      if (!companyId) throw new Error("Empresa não identificada");
      return ordersService.finalizarSeparacao(ordemId, companyId as string, user?.id);
    },
    onSuccess: (_, ordemId) => {
      qc.invalidateQueries({ queryKey: ["ordens-full"] });
