    ...
    (async () => {
      ...
      try {
        const [
          ...
          supabase.from("products").select("*", { count: "exact", head: true }).eq("company_id", companyId as string),
          supabase.from("sales").select("total_value").eq("company_id", companyId as string).eq("status", "completed"),
          supabase.from("ml_orders").select("total_amount, marketplace_fee, shipping_cost, status").eq("company_id", companyId as string),
          supabase.from("ml_order_items").select("ml_item_title, quantity, total_price, ml_order_id").order("total_price", { ascending: false }),
          supabase.from("ml_questions").select("status").eq("company_id", companyId as string).eq("status", "unanswered"),
