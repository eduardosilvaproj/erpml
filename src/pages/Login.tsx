...
import { LogIn, Loader2, Beaker } from "lucide-react";
...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err?.message === "timeout") {
        toast.error("O servidor demorou para responder. Tente novamente em instantes.");
      } else {
        toast.error(translateAuthError(err?.message || "Erro ao entrar"));
      }
    } finally {
      if (didError) setLoading(false);
    }
  };
  ...
  const handleCreateTestAccount = async () => {
    ...
    try {
      ...
      if (!result.success) {
        toast.error(result.message || "Não foi possível criar conta de teste no momento.");
        setTestAccountLoading(false);
        return;
      }

      toast.success("Conta de teste criada! Redirecionando...");
    } catch (err: any) {
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="grid grid-cols-2 gap-3 w-full">
                <Button
                  ...
                  disabled={googleLoading || appleLoading || loading || testAccountLoading}
                >
                  {googleLoading ? (
          ...
          <form onSubmit={handleLogin} noValidate>
            ...
            <CardFooter className="flex flex-col gap-3 pt-2">
              ...
              <Button
                type="button"
                variant="outline"
                className="w-full bg-amber-500/10 border-amber-500/20 text-amber-500 hover:bg-amber-500/20 transition-all duration-300"
                onClick={handleCreateTestAccount}
                disabled={loading || testAccountLoading || googleLoading || appleLoading}
              >
                {testAccountLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Beaker className="h-4 w-4 mr-2" />}
