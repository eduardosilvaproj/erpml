...
import { Search, TrendingUp, Package, Truck, BarChart3, Clock, Star, ArrowUpRight, Sparkles, MapPin, Phone, DollarSign, Lightbulb } from "lucide-react";
    ...
    <Card className="hover:shadow-lg transition-shadow">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-2">
          ...
          <Badge className={demandColor(product.demand_level)}>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          ...
          <span className={confidenceColor(product.confidence)}>
    ...
    <Card className="hover:shadow-lg transition-shadow">
      ...
      <CardContent className="space-y-2 text-sm">
        ...
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <span>{supplier.contact_hint}</span>
        </div>
        {supplier.min_order && (
          <div className="flex items-center gap-2 text-muted-foreground">
            <Package className="h-3.5 w-3.5 shrink-0" />
            <span>Mín: {supplier.min_order}</span>
          </div>
        )}
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Encontre produtos, fornecedores e oportunidades de mercado com IA
        </p>
      </div>
          ...
          <form onSubmit={handleSearch} className="flex gap-3">
            <div className="relative flex-1">
              ...
              <Input
                ...
                placeholder="Busque por produto, nicho ou categoria... Ex: 'capinha iPhone 15', 'eletrônicos importados'"
                className="pl-10"
        ...
        <Tabs defaultValue="products" className="space-y-4">
          <TabsList>
            <TabsTrigger value="products" className="flex items-center gap-1.5">
              ...
              Produtos ({searchResults.products?.length || 0})
            </TabsTrigger>
            <TabsTrigger value="suppliers" className="flex items-center gap-1.5">
              <Truck className="h-4 w-4" />
              Fornecedores ({searchResults.suppliers?.length || 0})
            </TabsTrigger>
          ...
          <TabsContent value="suppliers">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {searchResults.suppliers?.map((s, i) => (
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    <div className="rounded-xl bg-secondary/50 p-4">
                      ...
                      <p className="font-semibold text-foreground flex items-center gap-1">
                        ...
                        {searchResults.market_insights.trend}
                      </p>
                    </div>
                    <div className="rounded-xl bg-secondary/50 p-4">
        ...
        <CardHeader>
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => fetchTrending(nicheInput || undefined)}
                disabled={isTrendingLoading}
              >
                {isTrendingLoading ? (
