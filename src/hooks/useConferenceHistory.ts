...
export function useConferenceDetail(conferenceId: string | null) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const allItems = await fetchConferenceItemsRaw<ConferenceItemRow>(conferenceId!, "*", companyId as string);
...
export function useUpdateConferenceStatus() {
  ...
  return useMutation({
    mutationFn: async (input: { id: string; status: ConferenceStatus }) => {
      ...
      const { error } = await supabase.from("conferences").update(patch).eq("id", input.id).eq("company_id", companyId as string);
      if (error) throw error;
      return true;
    },
    onSuccess: (_d, vars) => {
