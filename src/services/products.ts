...
export const productsService = {
   ...
   */
  async fetchProducts(filters: ProductFilters | undefined, companyId: string) {
   ...
   */
  async createProduct(data: ProductFormData, companyId: string) {
    if (!companyId) {
      throw new Error("Nenhuma empresa identificada para o usuário. Faça login novamente ou complete o onboarding.");
    }
    ...
    try {
      ...
      // Normaliza mensagens de erro: prioriza o motivo retornado pela função
      const serverMessage =
        (validation && (validation.error || validation.message)) ||
        validationError?.message;

      if (validationError || !validation || !validation.valid) {
        const msg = serverMessage || "Erro na validação do produto";
        if (/empresa|company/i.test(msg) && /pertenc|member|membro/i.test(msg)) {
          throw new Error("Você não pertence a esta empresa. Verifique seu vínculo na equipe.");
        }
        throw new Error(msg);
      }
  ...
  async createSupplier(data: SupplierFormData, companyId: string) {
    if (!companyId) throw new Error("companyId é obrigatório");
