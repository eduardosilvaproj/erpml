...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!products || products.length === 0) {
      return new Response(JSON.stringify({ suggestions: [], explanation: "Nenhum produto encontrado." }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (candidatos.length === 0) {
      return new Response(JSON.stringify({ suggestions: [], explanation: "Nenhum produto atende ao critério." }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
...
Forneça também uma explicação curta (1-2 frases) sobre os critérios usados.`;
    ...
    if (!aiResp.ok) {
      ...
      if (aiResp.status === 429) {
        return new Response(JSON.stringify({ error: "Limite de IA atingido. Tente novamente em instantes." }), { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
