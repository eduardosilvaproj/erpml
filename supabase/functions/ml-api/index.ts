...
let cors: Record<string, string> = { "Access-Control-Allow-Origin": "*" };
...
async function getValidToken(supabase: any, userId: string) {
  ...
  if (!appId) missing.push("MERCADO_LIVRE_APP_ID");
  if (!clientSecret) missing.push("MERCADO_LIVRE_CLIENT_SECRET");
  if (missing.length) {
    throw new Error(`Credenciais do Mercado Livre não configuradas. Faltando: ${missing.join(", ")}`);
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "get-auth-url") {
      const appId = Deno.env.get("MERCADO_LIVRE_APP_ID");
      if (!appId) {
        return jsonResponse({ error: "Credenciais do Mercado Livre não configuradas. Faltando: MERCADO_LIVRE_APP_ID" }, 500);
      }
