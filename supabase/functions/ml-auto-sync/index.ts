...
Deno.serve(async (req) => {
  ...
  if (!appId) missingMl.push("MERCADO_LIVRE_APP_ID");
  if (!clientSecret) missingMl.push("MERCADO_LIVRE_CLIENT_SECRET");
  if (missingMl.length) {
    return new Response(JSON.stringify({ error: `Credenciais do Mercado Livre não configuradas. Faltando: ${missingMl.join(", ")}` }), { status: 500 });
  }
